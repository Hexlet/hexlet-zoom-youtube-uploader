...
export const prepareYoutubeTask = (server) => {
  ...
  return () => server.storage.records
    ...
    .then((items) => {
      ...
      const clientItemMap = filteredItems.map((item) => {
        const googleClient = server.googleClient.getBy({ owner: item.owner });
    ...
    .then((clientItemMap) => {
      const loadPromises = clientItemMap.map(([client, item]) => {
        itemsInProcessing.add(item.id);
        const { data } = item;
